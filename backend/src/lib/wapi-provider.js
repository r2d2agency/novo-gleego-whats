...
export async function sendImage(instanceId, token, phone, imageUrl, caption = '') {
  ...
  try {
    const response = await fetchWithRetry(
      ...
      // NÃO fazer retry em envio de mídia: se a W-API já aceitou mas demorou
      // para responder, um retry envia o mesmo arquivo duas vezes no WhatsApp.
      { retries: 0, baseDelay: 1000, timeout: 120000, label: 'wapi-sendImage' }
    );
...
export async function sendAudio(instanceId, token, phone, audioUrl) {
  ...
  try {
    const response = await fetchWithRetry(
      ...
      { retries: 0, baseDelay: 1000, timeout: 120000, label: 'wapi-sendAudio' }
    );
...
export async function sendVideo(instanceId, token, phone, videoUrl, caption = '') {
  ...
  try {
    const response = await fetchWithRetry(
      ...
      { retries: 0, baseDelay: 1000, timeout: 180000, label: 'wapi-sendVideo' }
    );
...
export async function sendDocument(instanceId, token, phone, documentUrl, filename = 'document') {
  ...
  try {
    const response = await fetchWithRetry(
      ...
      { retries: 0, baseDelay: 1000, timeout: 180000, label: 'wapi-sendDocument' }
    );
