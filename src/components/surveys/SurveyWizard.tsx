...
import { ArrowRight, ArrowLeft, Check, Star, Trash2, Plus } from "lucide-react";
import { FormField } from "@/hooks/use-external-forms";

const RESPONDENT_FIELD_KEYS = ["name", "nome", "phone", "whatsapp", "telefone", "email"];

// Loose shape covering both a SurveyTemplate (from the library) and an
// ExternalForm (when editing an existing survey) — both are structurally
// compatible with this, so either can be passed as-is.
interface SurveyWizardInitialData {
  id?: string;
  name?: string;
  description?: string;
...
interface SurveyWizardProps {
  ...
  initialData?: SurveyWizardInitialData | null;
  isEditing?: boolean;
}
...
function buildInitialState(initialData?: SurveyWizardProps["initialData"]) {
  ...
  return {
    ...
    welcome_message: initialData?.welcome_message || "Gostaríamos de ouvir sua opinião!",
    thank_you_message: initialData?.thank_you_message || "Obrigado por participar!",
...
export function SurveyWizard({ onClose, onSave, isSubmitting, initialData, isEditing }: SurveyWizardProps) {
  ...
  const handleSave = () => {
    ...
    const respondentFields: FormField[] = formData.collectRespondentInfo
      ? [
          { field_key: "name", field_label: "Seu nome", field_type: "text", is_required: false, position: 0 },
          { field_key: "phone", field_label: "Seu WhatsApp", field_type: "phone", is_required: false, position: 0 },
          { field_key: "email", field_label: "Seu e-mail", field_type: "email", is_required: false, position: 0 },
        ]
      : [];

    const allFields = [...formData.fields, ...respondentFields].map((f, i) => ({ ...f, position: i }));

    const { collectRespondentInfo, ...rest } = formData;
    onSave({ ...rest, fields: allFields });
  };
  ...
  return (
    <Card className="w-full max-w-2xl mx-auto border-orange-200 shadow-xl">
      <CardHeader className="bg-orange-50/50 border-b border-orange-100">
        <div className="flex justify-between items-center">
          <CardTitle className="text-xl flex items-center gap-2 text-orange-700">
            ...
            {isEditing ? "Editar Pesquisa" : "Assistente de Pesquisa"}
          </CardTitle>
      ...
      <CardContent className="py-6 space-y-4 max-h-[65vh] overflow-y-auto">
        {step === 1 && (
          <div className="space-y-4 animate-in fade-in slide-in-from-right-4">
            ...
            <div className="flex items-start gap-3 p-4 bg-slate-50 rounded-lg border">
              <input
                ...
                onChange={(e) => setFormData((prev) => ({ ...prev, collectRespondentInfo: e.target.checked }))}
                className="h-4 w-4 mt-0.5 rounded border-gray-300 text-orange-600 focus:ring-orange-500"
              />
              <div>
                <Label htmlFor="collect-respondent" className="cursor-pointer">
                  Perguntar nome, WhatsApp e e-mail de quem responde
                </Label>
      ...
      <CardFooter className="flex justify-between border-t border-orange-50 bg-orange-50/30 py-4">
        ...
        >
          {step === 3 ? (
            isSubmitting ? "Salvando..." : <><Check className="mr-2 h-4 w-4" /> {isEditing ? "Salvar alterações" : "Finalizar"}</>
          ) : (
